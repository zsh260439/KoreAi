...
export class KnowledgeService {
 ...
 async searchKnowledge(
...
): Promise<ApiResponse<KnowledgeSearchHit[]>> {
  ...
  //检索知识库
  const hits = await this.retrieveKnowledge(dto.knowledgeBaseId, query, 20);
 ...
 async askKnowledge(
  dto:KnowledgeAskInput,
    ...
    return ApiResponse.success(0, "问答成功", {
      answer,
      sources,
...
private async retrieveKnowledge(
 ...
 //全部chunk拉出来召回
  ...
  private async vectorRecall(
    ...
    //返回的是[文档，分数]
    const results =
      await this.knowledgeVectorStoreService.similaritySearchWithScore(
        query,
        limit,
        knowledgeBaseId,
      );
...
function ensureSupportedLocalTextFile(storagePath: string): void {
  ...
  if (![".txt", ".md"].includes(extension)) {
    throw new BadRequestException(
      "Only .txt and .md files are supported in this step",
    );
  }
}
...
function calcSimpleScore(content: string, keywords: string[]): number {
  ...
  const densityScore = Math.min(
    15,
    (matchedKeywordLength / Math.max(1, content.length)) * 100,
  );

  return Number(
    Math.min(100, coverageScore + countScore + densityScore).toFixed(2),
  );
}

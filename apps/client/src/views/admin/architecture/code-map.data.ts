...
export const functionDocs: FunctionDoc[] = [
  ...
  },
  {
    ...
    layer: 'llm',
    summary:
      '这是最接近 LLM 的地方。它会先发起一次模型流；普通模式直接把 chunk 映射成 answer_delta，think 模式则用 htmlparser2 解析 `<koreai_think>` 和 `<koreai_answer>` 两段标签，再映射成前端事件流。',
    code: [
    ...
    comments: [
      ...
      {
        title: 'think 模式走标签解析拆流',
        start: 22,
        end: 28,
        detail:
          '模型输出先被 extractStreamingMessageText 转成纯文本增量，再由 htmlparser2 驱动的 parseKnowledgeQaTaggedDelta 按 `<koreai_think>` 和 `<koreai_answer>` 两个标签拆成 thinking_delta 与 answer_delta。'
      }
    ],

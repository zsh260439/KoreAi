...
export const functionDocs: FunctionDoc[] = [
  ...
  },
  {
    ...
    code: [
      ...
      '    { role: "system", content: includeReasoning ? buildStreamingSystemPrompt(hits.length > 0) : buildSystemPrompt(hits.length > 0, false) },',
      '    { role: "user", content: includeReasoning ? buildStreamingUserPrompt(query, hits) : buildUserPrompt(query, hits, false) }',

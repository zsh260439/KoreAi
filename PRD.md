...
## 本轮新增目标
- 为 admin 新增独立导航页，用于显式管理知识库级的检索与问答运行参数。
- 让影响行为的关键参数不再隐藏在环境变量或 service 常量里，至少包括：
  - QA temperature
  - query analysis / rewrite temperature
  - preview topK
  - workspace topK
  - candidate limit 策略
  - 手动 BM25 / 向量融合权重
- 配置修改后，admin preview 与 workspace 问答都走同一份知识库级配置。

## 本轮新增目标补充：全局召回配置
- 补齐“未指定知识库时的全局召回配置”，避免系统存在全库召回路径，但后台只能配置单知识库参数。
- admin 参数页需要支持两种作用域：
  - 全局召回配置
  - 单知识库配置
- workspace 已存在“全库搜索（默认）”入口；当用户不选择知识库时，应明确走全局配置，而不是硬编码默认值。

## 本轮验收补充
- admin 侧边导航新增独立入口，可进入参数配置页。
- 参数配置页支持：
  - 切换配置作用域：全局 / 单知识库
  - 编辑全局召回配置
  - 选择目标知识库
  ...
  - 修改并保存参数
  - 恢复默认参数
- 后端不再在 QA / retrieval 主链路中硬编码上述运行参数。
- 保存后的参数能够影响：
  - `knowledgeBaseId` 为空的全局召回 / 问答链路
  - `POST /knowledge/search`
  - workspace chat 的召回与回答链路
